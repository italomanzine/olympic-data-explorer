...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  if (active && payload && payload.length) {
    ...
    const sortedAthletes = [...data.Athletes].sort((a, b) => {
      const medalOrder: Record<string, number> = { 'Gold': 3, 'Silver': 2, 'Bronze': 1, 'NA': 0, 'No Medal': 0 };
    ...
    return (
      <div className="bg-white p-3 border border-slate-200 shadow-xl rounded-lg text-sm z-50 max-w-[250px]">
        <div className="mb-2 border-b border-slate-100 pb-1">
            ...
            <p className="text-xs text-slate-500">{data.Athletes.length} {t('athletes_here')}</p>
        </div>
        ...
        <div className="space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar">
          {displayAthletes.map((atleta, idx) => (
            <div key={idx} className="text-xs border-b border-slate-50 last:border-0 pb-1 last:pb-0">
              ...
              <p className="text-slate-500 inline-block mr-2">{atleta.Sex === 'M' ? t('male') : t('female')}</p>
              {atleta.Medal !== 'NA' && atleta.Medal !== 'No Medal' && (
                <span className={`font-bold text-[10px] px-1 rounded ${
                  atleta.Medal === 'Gold' ? 'bg-yellow-100 text-yellow-700' : 
                  atleta.Medal === 'Silver' ? 'bg-slate-100 text-slate-600' : 'bg-orange-100 text-orange-700'
                }`}>
        ...
        {remaining > 0 && (
            <p className="text-xs text-slate-400 mt-2 italic">+ {remaining} {t('other_athletes')}</p>
        )}
      </div>
    );
  }
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 20, right: 20, bottom: 10, left: 30 }}>
          ...
          <XAxis 
            ...
            unit="kg" 
            stroke="#94A3B8"
            label={{ value: `${t('weight')} (kg)`, position: 'insideBottom', offset: -5, fill: '#94A3B8' }} 
          />
          <YAxis 
            type="number" 
            dataKey="Height" 
            ...
            domain={['auto', 'auto']} 
            stroke="#94A3B8"
            label={{ value: `${t('height')} (cm)`, angle: -90, position: 'insideLeft', fill: '#94A3B8', style: { textAnchor: 'middle' }, offset: 0 }}
          />
          <ZAxis type="number" dataKey="Athletes.length" range={[60, 400]} name="Qtd" />
