...
import ChartSkeletonLoader from "./ui/ChartSkeleton";
...
export default function Dashboard() {
  ...
  // Estados de loading individuais para cada componente
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      // Ativar loading individual para todos os componentes
      setLoadingStates({
        map: true,
        biometrics: true,
        evolution: true,
      ...
      try {
        ...
        // Carregar dados em paralelo mas atualizar UI conforme chegam
        const mapPromise = fetchMapStats(debouncedFilters);
        const bioPromise = fetchBiometrics(debouncedFilters);
        const tablePromise = fetchMedalTable(debouncedFilters);
        const topAthletesPromise = fetchTopAthletes(debouncedFilters);
        const genderPromise = fetchGenderStats(debouncedFilters);
        
        let evoPromise: Promise<any> | null = null;
        if (!evoRes) {
           evoPromise = fetchEvolution(evolutionFilters);
        } else {
          // Se já temos cache, atualiza imediatamente
          setEvolutionData(evoRes);
          setLoadingStates(prev => ({ ...prev, evolution: false }));
        }

        // Atualizar cada componente assim que seus dados chegarem
        mapPromise.then(res => {
          setMapData(res);
        ...
        bioPromise.then(res => {
          setBiometricsData(res);
          setLoadingStates(prev => ({ ...prev, biometrics: false }));
        }).catch(() => {
          setLoadingStates(prev => ({ ...prev, biometrics: false }));
        });
        
        tablePromise.then(res => {
          setMedalTableData(res);
          setLoadingStates(prev => ({ ...prev, medals: false }));
        }).catch(() => {
          setLoadingStates(prev => ({ ...prev, medals: false }));
        });
        
        topAthletesPromise.then(res => {
          setTopAthletesData(res);
          setLoadingStates(prev => ({ ...prev, topAthletes: false }));
        }).catch(() => {
          setLoadingStates(prev => ({ ...prev, topAthletes: false }));
        });
        
        genderPromise.then(res => {
          setGenderData(res);
          setLoadingStates(prev => ({ ...prev, gender: false }));
        }).catch(() => {
          setLoadingStates(prev => ({ ...prev, gender: false }));
        });
        ...
        if (evoPromise) {
          evoPromise.then(res => {
            ...
            setCachedData(evolutionCacheKey, res);
            setLoadingStates(prev => ({ ...prev, evolution: false }));
          }).catch(() => {
            setLoadingStates(prev => ({ ...prev, evolution: false }));
          });
        ...
        // Aguardar todas as promises para cache e prefetch
        const [mapRes, bioRes, tableRes, topAthletesRes, genderRes] = await Promise.all([
          mapPromise, bioPromise, tablePromise, topAthletesPromise, genderPromise
        ]);
        ...
        // Prefetch do próximo ano durante playback
        if (isPlaying) {
        ...
        // Em caso de erro, desativar todos os loadings
        setLoadingStates({
          map: false,
          biometrics: false,
          evolution: false,
          ...
          <div>
            ...
            <SearchableSelect
              options={availableSports.map(s => ({ 
                value: s, 
                label: s === "All" ? t('all_sports') : tSport(s)
              }))}
              ...
              <div className="flex-3 flex flex-col gap-6 min-w-0">
                  ...
                  <div className="flex-1 relative">
                    <div className="absolute inset-0 p-4">
                       {loadingStates.map ? (
                         <ChartSkeletonLoader 
                           variant="map" 
                           message={initialLoad ? "Conectando ao servidor..." : undefined}
                         />
                       ) : (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-[400px]">
                   ...
                   <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all hover:shadow-md h-full">
                    ...
                    <div className="flex-1 p-4 min-h-0 h-full">
                      {loadingStates.evolution ? (
                        <ChartSkeletonLoader 
                          variant="chart" 
                          message={initialLoad ? "Carregando evolução..." : undefined}
                        />
                      ) : (
                  ...
                  <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all hover:shadow-md h-full">
                    ...
                    <div className="flex-1 p-4 min-h-0 h-full">
                      {loadingStates.gender ? (
                        <ChartSkeletonLoader 
                          variant="pie" 
                          message={initialLoad ? "Carregando dados..." : undefined}
                        />
                      ) : (
                ...
                <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all hover:shadow-md h-[400px] shrink-0">
                  ...
                  <div className="flex-1 p-4 min-h-0">
                    {loadingStates.topAthletes ? (
                      <ChartSkeletonLoader 
                        variant="bar" 
                        message={initialLoad ? "Carregando atletas..." : undefined}
                      />
                    ) : (
                ...
                <section className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all hover:shadow-md h-[400px] shrink-0">
                  ...
                  <div className="flex-1 p-4 min-h-0 h-full">
                    {loadingStates.biometrics ? (
                      <ChartSkeletonLoader 
                        variant="chart" 
                        message={initialLoad ? "Carregando biometria..." : undefined}
                      />
                    ) : (
              ...
              <div className="flex-1 xl:max-w-[400px] flex flex-col gap-6">
                 ...
                 <div className={selectedAthlete ? '' : 'sticky top-6 h-full'}>
                    <MedalTable 
                        ...
                        isLoading={loadingStates.medals}
                    />
                 </div>
              </div>
